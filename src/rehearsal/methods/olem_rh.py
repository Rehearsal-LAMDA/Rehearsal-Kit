...
from rehearsal.models.order import OrderBasedStructuralLearner
...
class OLEMRhRehearsal:
    ...
    def fit(
        ...
        learner = OrderBasedStructuralLearner(
            ...
            parent_correlation_threshold=float(config_dict.get("parent_correlation_threshold", 1e-6)),
        )
    ...
    def suggest(self, observation: Mapping[str, float], task: AUFTask) -> DecisionResult:
        if self.model_ is None:
            raise RuntimeError("OLEMRhRehearsal.fit must be called before suggest.")
        start = time.perf_counter()
        ...
        diagnostics = {
            "method_family": "unpublished OLEM-Rh",
    ...
    def evaluate(self, task: AUFTask, n_samples: int) -> Mapping[str, Any]:
        ...
        rng = np.random.default_rng(None if self.seed is None else int(self.seed) + 104003)
...
UnpublishedOLEMRhRehearsal = OLEMRhRehearsal


def _candidate_rng(seed: int | None, candidate_index: int) -> np.random.Generator:
    if seed is None:
        return np.random.default_rng()
    return np.random.default_rng(int(seed) + 15485863 * (candidate_index + 1))
