...
Reference only in the original round:
...
Current addendum for this round:

- NeurIPS 2025 is no longer reference-only for this task. Port
  `Variance-Reduced Long-Term Rehearsal Learning with Quadratic Programming
  Reformulation` as one package method registered exactly as `mur`.
- Keep both GMuR and FarMuR behind the same adapter via a `variant` parameter;
  do not register legacy aliases.
- Implement a dedicated time-series linear additive SRM for
  `V_t = A V_t + B V_{t-1} + eps_t`; do not reuse static path-effect logic from
  `LinearGaussianSRM`.
- Keep `previous_works/07-NeurIPS 2025/` read-only and port only the algorithmic
  matrix/QP behavior into `src/rehearsal/`.
...
### Current Addendum: NeurIPS 2025 MUR
...
- `src/rehearsal/metrics/mur.py` when light shared helpers are useful
- `examples/mur/`
- `tests/test_mur.py`
- necessary registration/export updates and README reference rows

Tasks:

- Add a NumPy-only linear time-series SRM with instantaneous matrix `A`, lagged
  matrix `B`, Gaussian noise covariance, direct/config construction, theta-dict
  construction, and masked least-squares fitting from time-ordered data.
- Implement NeurIPS 2025 MUR matrix computation for `M`, `N`, `H`, and `F`,
  using the reverse-chronological sequence order
  `[z_{t+T}, ..., z_t]` and executing the last block as the current action.
- Implement bounded box-QP solving with robust pseudoinverse initialization,
  projected-gradient fallback, deterministic seeded restarts, rank diagnostics,
  and strict alteration-domain clipping.
- Add `MURRehearsal` / `NeurIPS2025MURRehearsal` with variants `gmur` and
  `farmur`, desired-region center handling, previous-state fallback diagnostics,
  model-based evaluation, and JSON-friendly decision diagnostics.
- Add toy and Bermuda seeded experiment configs following the existing batch
  runner contract. The Bermuda config uses the NeurIPS 2025 alterable set and
  `horizon=0` reference behavior, where GMuR and FarMuR degenerate to the same
  current action.

Acceptance criteria:

- `create_method("mur", {"variant": "gmur"})` and
  `create_method("mur", {"variant": "farmur"})` instantiate the same adapter
  class with normalized diagnostics.
- Focused tests cover fitting, suggesting, evaluating, invalid variants,
  deterministic seeds, bounds, center inference, fallback previous-state
  diagnostics, matrix values/shapes, current-block selection, rank-deficient
  fallback, candidate selection, and runner smokes.
- The README implemented-method table and Bermuda reference table include MUR,
  and `outputs/mur_bermuda_seed3.json` is generated by a README-style command.
- The full CPU test suite passes with:
